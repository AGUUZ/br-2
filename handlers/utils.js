...
module.exports = async (client) => {
  ...
  client.editPlayerMessage = async (channel) => {
      ...
      // هنا نقوم بتعطيل الحذف والتعديل
      // لا نقوم بفعل أي شيء إذا كانت nowplayingMsg = false
  

      // لا نريد تعديل الرسالة، لذلك لا نفعل شيئًا هنا
  };
  ...
  client.playembed = (guild) => {
    let embed = new EmbedBuilder()
      ...
      .setAuthor({
        name: `انضم إلى قناة صوتية واكتب اسم الأغنية أو أرسل رابط الأغنية فقط`,
     ...
     // )
      .setImage(
        guild.banner
          ...
          : `https://media.discordapp.net/attachments/982619933445869588/990981467528114246/BrandoBNR.png?ex=67053c69&is=6703eae9&hm=a98440d1b228f24e1015fd925efe7e6c1b4d191f331c2d7c5dc8ab7aac091230&=&format=webp&quality=lossless&width=840&height=473`
      )
      .setFooter({
        text: guild.name,
    ...
    queueembed.edit({
      embeds: [
        new EmbedBuilder()
          .setColor(client.config.embed.color)
          .setAuthor({
            name: `قائمه  التشغيل  -  [ ${queue.songs.length}  اغاني]`,
            iconURL: guild.iconURL({ dynamic: true }),
          })
          .setDescription(string.substring(0, 2048))
          .addFields([
            {
              name: `الاغنية الحالية`,
  ...
  client.HelpCommand = async (interaction) => {
    ...
    let help_embed = new EmbedBuilder()
      ...
      .addFields([
        {
          name: `<:1111768114405838928:1148945415551590441> Music`,
          value: "`ش-شغل` ' `س-قف` ' `ت-تخطي` ' `ص-صوت` '  `ك-كرر` ' " // استخدم value هنا
        },

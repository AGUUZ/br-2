    ...
    client.distube.on("addSong", async (queue, song) => {
      ...
      .send({
        embeds: [
          ...
          .setDescription(`☑ | Added track \`${song.name}\` <@${song.user.id}>`)
      ...
      queue.textChannel
        .send({
          embeds: [
            ...
            .setDescription(`☑ | Added playlist \`${playlist.name}\` <@${playlist.user.id}>`)
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`Queue ended! | No more music to play`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
          ...
          case "skip":
            {
              if (!channel) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} ⛔ You must join a voice channel to do this.`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
